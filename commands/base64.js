    ...
    execute(message, args, commandName) {
      ...
      var option = commandName
        try {
            switch(args[0]) {
                case 'encode':
              if (!args.slice(1).join(' ')) return message.reply('Yea but what do you even want to encode?');
              ...
              .addField('Encoded Text:', base64encode(args.slice(1).join(' ')), true)
              .setTimestamp()
              .setFooter('Beep Boop Bop! Im a bot made in discord.js!');
              message.channel.send(encodebase64);
              return;
                case 'decode':
                if (!args.slice(1).join(' ')) return message.reply('Hmmmmm Nah, what do you want to encode?');
                ...
                .addField('What It Said:', base64decode(args.slice(1).join(' ')), true)
                .setTimestamp()
                .setFooter('Beep Boop Bop! Im a bot made in discord.js!');
