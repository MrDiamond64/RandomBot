...
module.exports = {
      ...
      execute(message, args) {
        if (!message.member.hasPermission("MUTE_MEMBERS")) {
            message.reply('Uhmm NO! You dont have the right permissions to unmute people!!!')
        }
            let member = message.mentions.members.first();
            let reason = args[1]
            member.setMute(false, reason)
            .catch(error => message.reply(`${message.author} I couldn't unmute this person because of: ${error}`));
