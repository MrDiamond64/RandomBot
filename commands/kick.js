...
module.exports = {
  ...
  if (!message.member.hasPermission("KICK_MEMBERS")) {
    message.reply('Uhmm NO! You dont have the right permissions to kick people!!!')
    return;
  }
    message.react('🥾');
      let member = message.mentions.members.first();
        ...
        message.reply(' No No No. Self-harm is bad! Dont kick yourself!')
        return;} // smile

        if (message.mentions.users.first().id === botownerID) {
        message.reply('Hey! Dont Kick My Developer!')
        return; }

        if(!member)
          return message.reply("Umm I dont think thats a real person ");
		  if(!member.kickable) 
		    return message.reply("Error! I couldnt kick this person! Do I have kick perms?");
		  let reason = args.slice(1).join(' ');
		  if(!reason) reason = "No reason provided";
		  member.kick(reason)
		    .catch(error => message.reply(`Error ${message.author}! I couldn't kick because of : ${error}`));
      // Mod Logs
